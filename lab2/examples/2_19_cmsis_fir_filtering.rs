...
//! filtering operation using CMSIS-DSP library FIR function. Here we have a
//! digital input signal, sum of two sinusoidal signals with different
//! frequencies. This signal is represented with x array in main.c file.
//! Originally, one of these sinusoidal signals is filtered out using the
//! coefficients given in FIR_lpf_coefficients.h file. The output signal is
//! represented with y array and filter coefficients are stored in h array in
//! main.c file. User can replace FIR_lpf_coefficients.h file with
//! FIR_hpf_coefficients.h file to filter out other sinusoidal signal.
//!
...
//!
//! `cargo size --release --example 2_19_cmsis_fir_filtering`
//! `cargo embed --release --example 2_19_cmsis_fir_filtering`
