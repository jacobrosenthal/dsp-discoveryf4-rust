...
//! for the length of the vec. This gets better with const generics arrive
//! someday.
//!
...
fn main() -> ! {
    ...
    let unit_pulse = utils::unit_pulse::<U10>().collect::<heapless::Vec<f32, U10>>();
    dbgprint!("unit_pulse: {:?}", &unit_pulse[..]);

    let unit_step = utils::unit_step::<U10>().collect::<heapless::Vec<f32, U10>>();
    dbgprint!("unit_step: {:?}", &unit_step[..]);

    let unit_ramp = utils::unit_ramp::<U10>().collect::<heapless::Vec<f32, U10>>();
    dbgprint!("unit_ramp: {:?}", &unit_ramp[..]);

    let exponential = utils::exponential::<U10>().collect::<heapless::Vec<f32, U10>>();
    dbgprint!("exponential: {:?}", &exponential[..]);

    let sinusoidal = utils::sinusoidal::<U10>().collect::<heapless::Vec<f32, U10>>();
